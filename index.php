<?php
// Koneksi ke database
$koneksi = mysqli_connect('localhost', 'root', '', 'db_todo_list');
if (!$koneksi) {
...
if (isset($_POST['add_task'])) {
    ...
    if (!empty($task) && !empty($priority) && !empty($due_date)) {
        mysqli_query($koneksi, "INSERT INTO todolist(task, priority, due_date, status) VALUES('$task', '$priority', '$due_date', '0')");
...
if (isset($_POST['edit_task'])) {
    ...
    mysqli_query($koneksi, "UPDATE todolist SET task='$task', priority='$priority', due_date='$due_date' WHERE id=$id");
...
if (isset($_GET['complete'])) {
    $id = $_GET['complete'];
    mysqli_query($koneksi, "UPDATE todolist SET status=1 WHERE id=$id");
    echo "<script>alert('Data berhasil diperbarui');</script>";
    header('Location: index.php');
}
...
$result = mysqli_query($koneksi, "SELECT * FROM todolist ORDER BY status ASC, priority DESC, due_date ASC");
?>
...
<body>
    <div class="container mt-2">
        <h2 class="text-center">Kegiatan Kamu</h2>
        ...
        if (isset($_GET['edit'])) {
            $id = $_GET['edit'];
            $edit_query = mysqli_query($koneksi, "SELECT * FROM todolist WHERE id=$id");
